...
def get_all_polish_ptcl_imgs( movie_name,
    ...
    newnx = int(nxx * (1.24/0.885))
    newny = int(nyy * (1.24/0.885))

    nxx = newnx
    nyy = newny

    particle_imgs_in_movie = []
    mask = sp_utilities.model_circle((newnx / 2), newnx, newnx)
    globaldose = 1.277
    ...
    for i in range(zsize):
        ...
        dose = globaldose * (i)
        damageweights = localdose(dose, newnx)
        doseline = damageweights[:,0].tolist()
        current_frame = applyshifts(current_frame, int(round(shift_x[i])), int(round(shift_y[i])))
...
def giveshiftperparticles(trackingfile):
...
def applyshifts(image, shifx , shify):
    newimage = sp_fundamentals.fshift(image, shifx, shify)
    return newimage
...
for micro in enumerate(movie_names[ima_start:ima_end]):
    ...
    shiftperptcl = giveshiftperparticles(trackfile)
    ...
    for i in range(len(ave_particle_dosed)):
        ...
        ave_particle_dosed[i] = sp_fundamentals.resample(ave_particle_dosed[i], (0.885/1.24))
