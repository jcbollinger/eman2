...
if [ -n "${TRAVIS}" ];then
    ...
    conda create -n eman-deps-19.0 eman-deps-dev=19.0 boost=1.70 -c cryoem -c defaults -c conda-forge --yes --quiet
...
if [ -n "${CIRCLECI}" ];then
    ...
    conda install eman-deps-dev=19.0 boost=1.70 -c cryoem -c defaults -c conda-forge -q --yes
fi
